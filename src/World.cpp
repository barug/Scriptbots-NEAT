...
#ifdef HAVE_VTK
    Sim.setVtkDashboard(new VTKDashboard());
#endif
}
...
#ifdef HAVE_VTK
    Sim.setVtkDashboard(new VTKDashboard(inFile));
#endif
...
#ifdef HAVE_VTK
        Sim.vtkDashboard()->addPopulationData(num_herbs_carns.first, num_herbs_carns.second);
#endif
        ptr++;
        if(ptr == numHerbivore.size()) ptr = 0;
        removeShortLivedSpecies();
#ifdef HAVE_VTK
        Sim.vtkDashboard()->addSpeciesData(all_species);
#endif
    }
...
void World::printToFile(std::string path)
{
    ...
    outFile << agents.size() << std::endl;
    for (std::vector<Agent*>::iterator it = agents.begin(); it != agents.end(); ++it) {
        (*it)->printToFile(outFile);
    }

    outFile << innovations.size() << std::endl;
    for (std::vector<NEAT::Innovation*>::iterator it = innovations.begin(); it != innovations.end(); ++it) {
        (*it)->printToFile(outFile);
    }
...
#ifdef HAVE_VTK
    Sim.vtkDashboard()->saveToFile(outFile);
#endif
...
#ifdef HAVE_VTK
         Sim.vtkDashboard()->displayAgentInfo(agents[mini]);
#endif
     }
}
